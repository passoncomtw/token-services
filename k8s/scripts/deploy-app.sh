...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  部署 Token App API${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
kubectl apply -f configmaps/app-api-config.yaml
echo -e "${GREEN}✓ ConfigMap 已更新${NC}"
echo ""

# 部署 Deployment 和 Service
echo -e "${YELLOW}部署 App API...${NC}"
...
kubectl get deployment token-app-api -n passontw-services
kubectl get pods -l app=token-app-api -n passontw-services
echo ""

echo -e "${GREEN}✓ App API 部署完成${NC}"
