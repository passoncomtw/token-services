...
DOCS_DIR_MERCHANT := cmd/pos-merchant-api/internal/docs
...
build-pos-merchant-swagger: ## 生成 pos-merchant-api 的 Swagger 文檔
	@echo "📚 生成 pos-merchant-api Swagger 文檔..."
	@if ! command -v $(GOPATH)/bin/swag >/dev/null 2>&1; then \
		echo "⚠️  swag 未安裝，正在安裝..."; \
		go install github.com/swaggo/swag/cmd/swag@latest; \
	fi
	@mkdir -p $(DOCS_DIR_MERCHANT)
	@cd cmd/pos-merchant-api && $(GOPATH)/bin/swag init \
		-g main.go \
		-o internal/docs \
		--parseDependency \
		--parseInternal \
...
.PHONY: run-pos-merchant-api
run-pos-merchant-api: ## 使用 air 執行 pos-merchant-api
	@echo "📚 檢查 swagger 文檔..."
	@if [ ! -d "$(DOCS_DIR_MERCHANT)" ]; then \
		echo "⚠️  swagger 文檔不存在，正在生成..."; \
		$(MAKE) build-pos-merchant-swagger; \
	fi
	@echo "🔥 使用 air 啟動 pos-merchant-api..."
	@if ! command -v $(GOPATH)/bin/air >/dev/null 2>&1; then \
		echo "⚠️  air 未安裝，正在安裝..."; \
		go install github.com/air-verse/air@latest; \
	fi
	@cd cmd/pos-merchant-api && $(GOPATH)/bin/air
